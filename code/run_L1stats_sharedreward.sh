...
DISK_THRESHOLD=95 # percent usage threshold for disk
MEMORY_THRESHOLD=95 # percent usage threshold for memory
  ...
  for model in 2; do
    for sub in `cat ${scriptdir}/newsubs.txt`; do
      for run in `seq $nruns`; do
        ...
        # Check disk space and memory before starting each subject
        check_disk_space
        check_memory
        ...
        # Run the main script for the subject
