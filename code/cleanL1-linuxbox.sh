...
#for sub in 112 134 140 141 143 144 155; do
        ...
        if [ ${#sub} -eq 3 ]; then
			...
			if [ "$ppi" == "ecn" -o  "$ppi" == "dmn" ]; then
				...
				if [ "$ppi" == "0" ]; then
					TYPE=act
					OUTPUT=${MAINOUTPUT}/L1_task-${TASK}_model-2_type-${TYPE}_run-${run_padded}_sm-${sm}
				else
					TYPE=ppi
					OUTPUT=${MAINOUTPUT}/L1_task-${TASK}_model-2_type-${TYPE}_seed-${ppi}_run-${run_padded}_sm-${sm}
				fi
			fi
