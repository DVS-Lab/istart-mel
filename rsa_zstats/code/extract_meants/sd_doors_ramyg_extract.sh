...
dest_dir="/ZPOOL/data/projects/istart-mel/data/socialdoors/ramyg_doors"

# Iterate through each subject folder in the source directory
for sub_dir in "${source_dir}/doors/"sub-*; do
    sub=$(basename "$sub_dir")
    echo "${sub} pulled successfully"
    
    # Define mask path
    mask="/ZPOOL/data/projects/istart-mel/rsa_zstats/masks/rHarvardOxford_right-amygdala_nan_thr_bin.nii.gz"
    ...
    for file in $input_files; do
        ...
        cp "$file" "${dest_dir}/${sub}_doors_zstat${number}.nii.gz"
        # Running fslmeants command with the renamed file
        fslmeants -i "${dest_dir}/${sub}_doors_zstat${number}.nii.gz" \
                  -m "${mask}" \
                  -o "${dest_dir}/${sub}_ramyg_doors_zstat${number}.txt"
