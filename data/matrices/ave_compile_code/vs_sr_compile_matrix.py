...
data_dir = '/ZPOOL/data/projects/istart-mel/data/sharedreward/vs_sharedreward'
...
zstat_order = ['fri_win', 'fri_loss', 'str_win', 'str_loss', 'comp_win', 'comp_loss']
...
for subject_id in subjects_to_include:
    ...
    for idx, zstat_type in enumerate(zstat_order, start=1):
        filename = f'sub-{subject_id}_vs_sharedreward_zstat{idx}.txt'
...
data_df.to_csv('vs_sharedreward_matrix.csv', index=False)

print("Data saved as 'vs_sharedreward_matrix.csv'.")
